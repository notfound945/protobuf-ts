...
async function main() {
    ...
    // search for @console-pbts/plugin in node_modules and add --proto_path argument
    ...
    if (child.error) {
        throw new Error("@console-pbts/protoc was unable to spawn protoc. " + child.error);
    }
...
async function ensureInstalled(version) {
    ...
    if (version === "latest" || version === undefined) {
        ...
        try {
            ...
            throw new Error(`@console-pbts/protoc failed to retrieve latest protoc version number: ${e}`);
        }
    ...
    try {
        ...
        throw new Error(`@console-pbts/protoc failed unzip the downloaded protoc release v${version}: ${e}`);
    }

    // sanity check
    let installed = listInstalled().find(i => i.name === releaseName);
    if (!installed) {
        throw new Error(`@console-pbts/protoc failed to install protoc v${version}.`);
    }

    // finished
    console.info(`@console-pbts/protoc installed protoc v${installed.version}.`);
    return installed;
}
